...
See `IMPLEMENTATION_PLAN.md`. The project is in Phase 22 — the presentation pass closing the engine→screen gap. Done: 22A save integrity (`runtime.rpy` `fh` holder keeps the unpicklable session out of Ren'Py's store; engine state rides native saves as the `fh_save_blob` JSON string, rebuilt in `after_load`; rollback disabled), 22B event routing, 22C generalised choice menu, 22E category template pools (zero generic-only blueprints), 22F match narration first pass (`narrate_match_phase`, `self_evaluation_line`), 22G lint gate. 22D is partial: engine scene intros (`scene_intro`) and sprite wiring (`focal_character` + `show_character`) are in; remaining work is authored `ChoiceNode` labels for all 55 blueprints and in-phase match event hooks. Run `scripts/check.sh` (pytest + renpy lint) before committing changes under `game/scripts/`.
